        ...
        for opp in [(game.RandomPlayer,0),(game.AILook1Player,1),(game.AILook1Player,5)]:
            for episode in range(0,50):
                ...
                if opp[0].__name__ == "RandomPlayer":
                    board.setPlayers(game.AITrainingReduFeatPlayer("1",board,model,3), opp[0]("2"))
                else:
                    board.setPlayers(game.AITrainingReduFeatPlayer("1",board,model,3), opp[0]("2",board,opp[1]))
...
def run(config_file):
    ...
    p.add_reporter(neat.Checkpointer(generation_interval=None, time_interval_seconds=3600, filename_prefix='RF3-PSR-'))
