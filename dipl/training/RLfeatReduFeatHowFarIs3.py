...
def trainModelWithDMC(p_boardSizeX, p_boardSizeY, p_episodes, savePath, p_temperature=20.0, p_gamma=0.9, howFar=3, rewardFunc=game.rewardFunc2, selfPlayFixOpp=False, startingModel=None, opponentConstr=None, opponentStrength=None):

    model = startingModel

    if startingModel is None:
        model = Sequential()
        model.add(Dense(70, kernel_initializer='lecun_uniform', activation='relu', input_shape=(4+(2*howFar+1)*(2*howFar+1),)))
...
trainModelWithDMC(p_boardSizeX=6,p_boardSizeY=6,p_episodes=50000,savePath='resultsPSR/feat/modelReduFeatHowFarIs3',startingModel=None,opponentConstr=game.AILook1Player,opponentStrength=5)
