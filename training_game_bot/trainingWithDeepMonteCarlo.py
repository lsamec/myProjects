...
def trainModelWithDMC(p_boardSizeX, p_boardSizeY, p_episodes, savePath, p_temperature=20.0, p_gamma=0.9, howFar=2, rewardFunc=game.rewardFunc2, selfPlayFixOpp=False, startingModel=None, opponentConstr=None, opponentStrength=None):

    model = startingModel

    if startingModel is None:
        model = Sequential()
        model.add(Dense(40, kernel_initializer='lecun_uniform', activation='relu', input_shape=(4+(2*howFar+1)*(2*howFar+1),)))
    ...
    batchSize = 1 #initially 40
    buffer = 1 # initially 80
    replay = []
    h = 0
    updateStep = 0
    for i in range(episodes):
        ...
        if(opponentConstr is None):
            if(selfPlayFixOpp == True):
                ...
                board.setPlayers(game.AITrainingReduFeatPlayer("1",board,model,howFar), game.AITrainingReduFeatPlayer("2",board,oppModel,howFar))
            else:
                 board.setPlayers(game.AITrainingReduFeatPlayer("1",board,model,howFar), game.AITrainingReduFeatPlayer("2",board,model,howFar))
        else:
            try:
                opp = opponentConstr("2",board,opponentStrength)
            except TypeError:
                opp = opponentConstr("2")
            board.setPlayers(game.AITrainingReduFeatPlayer("1",board,model,howFar), opp)
        ...
        while(board.checkGameStatus() == 0):
            ...
            state = board.to01ReducedFeatures(board.player1, howFar).reshape(1,4+(2*howFar+1)*(2*howFar+1))
        ...
        for stepNum in range(len(rewardList)):
            ...
            if (len(replay) < buffer):
                ...
                for memory in minibatch:
                    ...
                    X_train.append(state.reshape(4+(2*howFar+1)*(2*howFar+1),))
...
trainModelWithDMC(p_boardSizeX=6,p_boardSizeY=6,p_episodes=50000,p_temperature=5.0,savePath='models/temp/modelJustFinishedMCReduPs2',startingModel=None,opponentConstr=game.AILook1Player,opponentStrength=1)
